...
const CameraScreen = React.memo(() => {
  ...
  const pickImage = async () => {
    try {
      ...
      if (!result.cancelled) {
        ...
        // User cancelled image selection, trigger a refresh
      ...
      >
        ...
        >
          ...
          <TouchableOpacity onPress={() => setCapturedImage(null)}>
            <Text style={{ color: "white", marginTop: 10 }}>Remove Image</Text>
          </TouchableOpacity>
        </View>
      </LongPressGestureHandler>
    </View>
  ...
  return (
    <GestureHandlerRootView key={refreshKey.toString()} style={{ flex: 1 }}>
      <View style={{ flex: 1 }}>
        <Camera
          style={{ flex: 1 }}
          type={type}
        ...
        >
          {capturedImage ? renderCapturedImage() : renderCameraButtons()}
        </Camera>
      </View>
    </GestureHandlerRootView>
  );
